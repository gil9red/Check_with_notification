...
from common import (
    init_db,
    log,
    append_list_games_which_denuvo_is_removed,
    append_list_games,
    db_create_backup,
)
...
while True:
    try:
        ...
        log.debug(
            "games_without_denuvo (%s): %s",
            len(games_without_denuvo),
            text_games_without_denuvo,
        )

        changed_1 = append_list_games_which_denuvo_is_removed(
            games_without_denuvo, need_notification
        )
